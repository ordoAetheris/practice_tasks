 ...
 * <ul>
 *   <li><b>⭐ Конкурентный recordPrice</b> (тики из многих потоков) → thread-safety per-symbol; BigDecimal для цены (не double).</li>
 *   <li>Эффективное min/max/moving-average по окну (монотонный deque / скользящая сумма, не пересчёт); top-gainers/losers = heap; данные хронологически (бинпоиск по времени).</li>
 * </ul>
 */
