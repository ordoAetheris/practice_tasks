 ...
 * <ul>
 *   <li><b>⭐ Атомарность per-client</b> — несколько потоков на один clientId: для лимитера это КОРРЕКТНОСТЬ, не опция (CAS/лок на bucket, иначе over-admit). [web: thread-safe rate limiter — ключевая тема].</li>
 *   <li><b>Fixed-window boundary burst</b> (2× на стыке окон — почему sliding чинит); sliding LOG vs COUNTER (точность vs память); <b>unbounded clientId-map → утечка памяти → eviction неактивных</b>; распределённый (Redis + Lua-атомарность).</li>
 * </ul>
 */
