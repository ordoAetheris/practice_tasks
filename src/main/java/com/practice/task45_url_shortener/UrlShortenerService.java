 ...
 * <ul>
 *   <li><b>⭐ Гонка генерации/уникальности:</b> два shorten разом → коллизия авто-кода; customCode putIfAbsent (два кастома разом); счётчик переходов под конкуррентой (resolve → count = atomic).</li>
 *   <li>Генерация кода: counter+base62 vs random+retry vs KGS; распределённость/шардинг; deactivate под конкурентным resolve.</li>
 * </ul>
 */
