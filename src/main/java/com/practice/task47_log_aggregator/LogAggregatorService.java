 ...
 * <ul>
 *   <li><b>⭐ Высококонкурентный ingest:</b> много потоков пишут логи → thread-safety + монотонный порядок по timestamp (concurrent skip-list / lock); backpressure при заливе.</li>
 *   <li>Эффективный time-range (сортированная структура / бинпоиск по времени, не скан); getErrorRate по скользящему окну; findPatterns = прекомпил Pattern.</li>
 * </ul>
 */
